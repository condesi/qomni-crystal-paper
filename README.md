...
Standard conditional logic prevents SIMD vectorization. QOMN compiles conditions to float multiplications:
...
**Theorem:** For any oracle `f(x)` compiled by QOMN, bit-exact identical output is produced on every invocation for the same input `x` on the same architecture.
...
All tests are in [condesi/qomn](https://github.com/condesi/qomn):
...
```bash
# Run all (requires QOMN server on :9001)
...
├── arxiv/
│   └── main.tex            ← Updated LaTeX paper (QOMN v3.2)
├── qomn/
│   ├── SPEC.md             ← Language specification
│   ├── LANGUAGE_GUIDE.md   ← Programmer's guide
│   ├── compiler/           ← Embedded compiler snapshot
│   └── examples/           ← .qomn example oracles
├── code/
│   ├── qomn_pack.py     ← (v1) Ternary packing utility
│   └── qomn_synth_hidraulica.py ← (v1) Physics oracle synthesis
...
```bibtex
@misc{rojas2026qomn,
  title  = {QOMN v3.2: Deterministic Engineering Oracle Engine with JIT Compilation and AVX2 SIMD},
